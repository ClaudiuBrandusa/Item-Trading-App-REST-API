...
public class TradeItemTests
{
    ...
    private readonly List<TradeContent> collection;
    private readonly List<TradeItem> tradeItems;

    public TradeItemTests()
    {
        collection = new List<TradeContent>();
        tradeItems = new List<TradeItem>();
        var tradeItemRepositoryMock = TestingUtils.CreateRepositoryMock<TradeContent, ITradeItemRepository> (collection);
        ...
        tradeItemRepositoryMock.Setup(repo => repo.GetTradeContentCachedAsync(It.IsAny<string>(), It.IsAny<string>()))
            .ReturnsAsync((string tradeId, string itemId) =>
            {
                return GetTradeContent(tradeId, itemId);
            });

        tradeItemRepositoryMock.Setup(repo => repo.GetTradeIdsUsingItemCachedAsync(It.IsAny<string>()))
            .ReturnsAsync((string itemId) =>
            {
                return GetTradeIdsUsingItemId(itemId);
            });

        tradeItemRepositoryMock.Setup(repo => repo.ListTradeItemsCachedAsync(It.IsAny<string>(), It.IsAny<Func<string, Task<string>>>()))
            .ReturnsAsync((string tradeId, Func<string, Task<string>> getItemNameFunc) =>
            {
                return collection.Where(x => x.TradeId == tradeId)
                    .Select(content => _mapper.AdaptToType<TradeContent, TradeItem>
                        (content, (nameof(TradeItem.Name), content.ItemId)))
                    .ToArray();
            });

        #endregion MediatorMocks

        _sut = new TradeItemService(tradeItemRepositoryMock.Object, cacheServiceMock.Object, mediatorMock.Object, _mapper);
    }
    ...
    private TradeContent? GetTradeContent(string tradeId, string itemId)
    {
