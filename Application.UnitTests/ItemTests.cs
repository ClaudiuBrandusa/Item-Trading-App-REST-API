...
public class ItemTests
{
    ...
    public ItemTests()
    {
        ...
        var itemRepositoryMock = TestingUtils.CreateRepositoryMock<Item, IItemRepository>(collection);
        ...
        itemRepositoryMock.Setup(repo => repo.GetItemEntityCachedAsync(It.IsAny<string>(), It.IsAny<bool>()))
            .ReturnsAsync((string itemId, bool setCache) =>
            {
                return collection.FirstOrDefault(x => x.ItemId == itemId);
            });

        itemRepositoryMock.Setup(repo => repo.ListItemsCachedAsync())
            .ReturnsAsync(() => collection.ToArray());

        #endregion MediatorMocks

        _sut = new ItemService(itemRepositoryMock.Object, cacheServiceMock.Object, senderMock.Object, publisherMock.Object, TestingUtils.GetMapper());
    }
